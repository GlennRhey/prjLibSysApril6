...
namespace prjLibrarySystem
{
    public partial class SuperAdminMembers : System.Web.UI.Page
    {
        ...
        public static List<object> GetSearchSuggestions(string query)
        {
            ...
            {
                DataTable dt = DatabaseHelper.ExecuteQuery(@"
                    SELECT TOP 10 UserID, FullName, Email
                    FROM tblUsers
                    WHERE Role = 'Admin'
                      AND (FullName LIKE @Query OR Email LIKE @Query OR UserID LIKE @Query)
                    ORDER BY FullName",
                ...
                foreach (DataRow row in dt.Rows)
                {
                    suggestions.Add(new 
                    {
                        ...
                        subText = $"ID: {row["UserID"]}"
                    });
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Error getting admin suggestions: {ex.Message}");
            }
