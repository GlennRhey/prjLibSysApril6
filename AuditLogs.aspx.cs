...
namespace prjLibrarySystem
{
    public partial class AuditLogs : System.Web.UI.Page
    {
        ...
        public static List<object> GetSearchSuggestions(string query)
        {
            ...
            try
            {
                var queryParam = new SqlParameter[] { new SqlParameter("@Query", "%" + query + "%") };

                // User suggestions
                DataTable userDt = DatabaseHelper.ExecuteQuery(@"
                    SELECT DISTINCT TOP 5 UserName 
                    FROM tblAuditLogs 
                    WHERE UserName LIKE @Query
                    ORDER BY UserName", queryParam);
                ...
                foreach (DataRow row in userDt.Rows)
                {
                    suggestions.Add(new 
                    {
                        ...
                        displayText = $"<strong>{row["UserName"]}</strong> - User actions",
                        subText = "User"
                    });
                }

                // Action suggestions
                DataTable actionDt = DatabaseHelper.ExecuteQuery(@"
                    SELECT DISTINCT TOP 5 Action 
                    FROM tblAuditLogs 
                    WHERE Action LIKE @Query
                    ORDER BY Action",
                    new SqlParameter[] { new SqlParameter("@Query", "%" + query + "%") });

                foreach (DataRow row in actionDt.Rows)
                {
                    suggestions.Add(new 
                    {
                        value = row["Action"].ToString(),
                        displayText = $"<strong>{row["Action"]}</strong> - Action",
                        subText = "Action"
                    });
                }

                // Table suggestions
                DataTable tableDt = DatabaseHelper.ExecuteQuery(@"
                    SELECT DISTINCT TOP 5 AffectedTable 
                    FROM tblAuditLogs 
                    WHERE AffectedTable LIKE @Query
                    ORDER BY AffectedTable",
                    new SqlParameter[] { new SqlParameter("@Query", "%" + query + "%") });
                ...
                foreach (DataRow row in tableDt.Rows)
                {
                    suggestions.Add(new 
                    {
                        value = row["AffectedTable"].ToString(),
                        displayText = $"<strong>{row["AffectedTable"]}</strong> - Table operations",
                        subText = "Table"
                    });
                }
            }
