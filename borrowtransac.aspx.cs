...
namespace prjLibrarySystem
{
    public partial class Loans : System.Web.UI.Page
    {
        ...
        public static List<object> GetSearchSuggestions(string query)
        {
            ...
            {
                DataTable dt = DatabaseHelper.ExecuteQuery(@"
                    SELECT TOP 10 t.BorrowID, b.Title, m.FullName 
                    FROM tblTransactions t
                    INNER JOIN tblBooks b ON t.ISBN = b.ISBN
                    INNER JOIN tblMembers m ON t.MemberID = m.MemberID
                    WHERE (CAST(t.BorrowID AS NVARCHAR) LIKE @Query OR b.Title LIKE @Query OR m.FullName LIKE @Query)
                ...
                foreach (DataRow row in dt.Rows)
                {
                    suggestions.Add(new 
                    {
                        value = row["BorrowID"].ToString(),
                        displayText = $"<strong>{row["BorrowID"]}</strong> - {row["Title"]}",
                        subText = $"Member: {row["FullName"]}"
                    });
                }
            }
            catch (Exception ex)
