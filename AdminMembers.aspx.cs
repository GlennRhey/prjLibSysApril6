...
namespace prjLibrarySystem
{
    public partial class AdminMembers : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(role) || (role != "Admin" && role != "Super Admin")) { Response.Redirect("Login.aspx"); return; }

            var sidebar = SidebarHelper.GetSidebar(role, "members");
            if (sidebar != null) {
                litSidebar.Text = sidebar;
            } else {
                litSidebar.Text = "<nav class='col-12 col-md-3 col-lg-2 d-block' style='min-height:100vh;background:linear-gradient(135deg,#8b0000 0%,#b11226 100%);'><div class='position-sticky pt-3'><div style='text-align:center;margin-bottom:1.5rem;padding:0 10px;'><h4 style='color:white;margin:0;'>Admin Portal</h4></div></div></nav>";
            }
        ...
        public static List<object> GetSearchSuggestions(string query)
        {
            ...
            {
                DataTable dt = DatabaseHelper.ExecuteQuery(@"
                    SELECT TOP 10 m.MemberID, m.FullName, u.Email, u.UserID
                    FROM tblMembers m
                    INNER JOIN tblUsers u ON m.UserID = u.UserID
                    WHERE u.Role = 'Member'
                      AND (m.FullName LIKE @Query OR u.Email LIKE @Query OR u.UserID LIKE @Query OR CAST(m.MemberID AS NVARCHAR) LIKE @Query)
                    ORDER BY m.FullName",
                    new SqlParameter[] { new SqlParameter("@Query", "%" + query + "%") });
                ...
                foreach (DataRow row in dt.Rows)
                {
                    suggestions.Add(new 
                    {
                        ...
                        displayText = $"<strong>{row["FullName"]}</strong> - {row["Email"]}",
                        subText = $"ID: {row["MemberID"]}"
                    });
                }
            }
            catch (Exception ex)
