    ...
    <form id="form1" runat="server">
        ...
        <script>
            ...
            function initializeAutocomplete() {
                var searchInput = document.getElementById('<%= txtSearchLoan.ClientID %>');
            ...
            function selectSuggestion(value) {
                ...
                // Trigger search
                __doPostBack('<%= btnSearchLoan.UniqueID %>', '');
            }

            function fetchSearchSuggestions(query) {
                // Use real web service for transaction suggestions
                fetch('SearchSuggestions.asmx/GetTransactionSuggestions', {
                    method: 'POST',
            ...
            function showDetailsModal(loanId) {
                fetch('borrowtransac.aspx/GetLoanDetails', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json;charset=utf-8' },
                ...
                .then(function(data) {
                    var d = data.d;
                    if (d) {
                        document.getElementById('dtlLoanId').textContent = d.LoanID || '';
                        document.getElementById('dtlBookTitle').textContent = d.BookTitle || '';
                        document.getElementById('dtlMemberName').textContent = d.MemberName || '';
                        document.getElementById('dtlBorrowDate').textContent = d.BorrowDate ? new Date(d.BorrowDate).toLocaleDateString() : '';
                        document.getElementById('dtlDueDate').textContent = d.DueDate ? new Date(d.DueDate).toLocaleDateString() : '';
                        document.getElementById('dtlReturnDate').textContent = d.ReturnDate ? new Date(d.ReturnDate).toLocaleDateString() : 'Not returned';
                        document.getElementById('dtlStatus').innerHTML = '<span class="badge ' + (statusColors[d.status] || 'bg-secondary') + '">' + (d.status) + '</span>';
                        new bootstrap.Modal(document.getElementById('detailsModal')).show();
                    }
                })
                .catch(function(err) { console.error(err); alert('Error loading loan details'); });
